        ...
        try:
            ...
            if EnableNotifications.lower().strip() == "yes" and APIKEY and MobileNumber:
                for number in MobileNumber.split(","):
                    ...
                    url = "https://api.clockworksms.com/http/send.aspx?key=%s&to=%s&from=PoshC2&content=[%s]%%20-%%20NewImplant:%%20%s\\%s @ %s" % (APIKEY, number, NotificationsProjectName, self.Domain, self.User, self.Hostname)
                    url = url.replace(" ", "+")
